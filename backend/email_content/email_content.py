...
def generate_challenge_created_content(candidate):
    return f"""<table border="0" align="center" cellpadding="0" cellspacing="0"><h2 style="font-weight:normal">Hi, {candidate.first_name}! You have been assigned a Technical Challenge!</h2>
            <br></br>
            ...
            <br></br>
            <a href="https://localhost:3000/login/"><button style="outline:none; background: #EF485C; border-radius: 40px; width: 180px; height: 40px; color: white; font-size: 16px; border: none;" >Go to login</button></a>
            <br></br>
            <br></br>
            <h3 style="font-weight:normal">If after clicking the button you are not redirected to the challenge please send us an email to @</h3>
...
def generate_challenge_score_failed_creator(creator, candidate, challenge):
    return f"""<h3>Hi, {creator.first_name}! The candidate {candidate.first_name} {candidate.last_name} got {challenge.score}% on the Technical Challenge!</h3>
...
def generate_challenge_score_needs_review_candidate(candidate, challenge):
    return f"""<h2 style="font-weight:normal">Hi, {candidate.first_name}! You got {challenge.score}% on your Technical Challenge.</h2>
...
def generate_challenge_score_passed_candidate(candidate):
    return f"""<h2 style="font-weight:normal">Hi, {candidate.first_name}! Congratulations! You passed the Technical Challenge!</h2>
...
def generate_new_user(new_user):
    return f"""<table border="0" align="center" cellpadding="0" cellspacing="0"><h2 style="font-weight:normal">Hi, {new_user.first_name}! Welcome to your Technical Challenge Platform!</h2>
            <br></br>
            <h3 style="font-weight:normal">In order to start using the platform, you need to verify your email.</h3>
            <h3 style="font-weight:normal">Please click the button below to confirm your user information:</h3>
            <br></br>
            <a href="https://localhost:3000/verification/{new_user.id}?email={new_user.email}&first_name={new_user.first_name}&last_name={new_user.last_name}&phone={new_user.phone}"><button style="outline:none; background: #EF485C; border-radius: 40px; width: 180px; height: 40px; color: white; font-size: 16px; border: none;" >Go to verification</button></a>
            <br></br>
            <br></br>
